...
with app.app_context():     # Need to refactor to use Flask 3
    connect_db(app)
    db.create_all()
...
def add_song_to_playlist(playlist_id):
    ...
    songs = db.session.query(Song.id, Song.title).filter(Song.id.notin_(curr_on_playlist)).all()
    form.song.choices = [(row.id, row.title) for row in songs]
